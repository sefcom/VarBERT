...
# vocab_size = 150001 #last for unknown 0-50K
# var_size = 2
...
class RobertaLMHead3(nn.Module):
    ...
    def __init__(self,config):
        ...
        self.dense = nn.Linear(config.hidden_size, config.hidden_size)
        self.layer_norm = nn.LayerNorm(config.hidden_size, eps=config.layer_norm_eps)
        self.decoder = nn.Linear(config.hidden_size, config.out_origin_size, bias=False)
        self.bias = nn.Parameter(torch.zeros(config.out_origin_size))
...
class RobertaForMaskedLMv2(RobertaForMaskedLM):
    ...
    def __init__(self, config):
        ...
        self.out_origin_size = config.out_origin_size
        self.init_weights()
    ...
    def forward(
        ...
        if labels is not None:
            ...
            masked_lm_loss_vartype = loss_fct_type(prediction_scores_vartype.view(-1, self.out_origin_size), decomp_labels.view(-1))

        masked_loss = masked_lm_loss + masked_lm_loss_vartype

        output = (prediction_scores, prediction_scores_vartype) + outputs[2:]
...
class CMLDataset(Dataset):
    ...
    def __getitem__(self, i):
        ...
        if len(input_ids) >= block_size-2:
            ...
            decompids = decompids + [tokenizer.pad_token_id]*(self.block_size-2-len(decompids))
        
        input_ids = tokenizer.build_inputs_with_special_tokens(input_ids)
        labels = tokenizer.build_inputs_with_special_tokens(labels)
#         decompids = tokenizer.build_inputs_with_special_tokens(decompids)
        decompids = [-100] + decompids + [-100]
        ...
        try:
            ...
            decompids = torch.tensor(decompids, dtype=torch.long)
            mask_idxs = (input_ids==tokenizer.mask_token_id).bool()
            labels[~mask_idxs]=-100
            labels = labels.reshape(labels.size()[0],1)
            decompids = decompids.reshape(decompids.size()[0],1)
            
#             nmask = decompids > 1
#             decompids[nmask]=-100
            
            labels = torch.cat((labels, decompids), -1)

        except:
            print("Idx:",i,"Len of Examples:",len(self.examples))
...
def load_and_cache_examples(args, tokenizer, evaluate=False):
    ...
    return CMLDataset(tokenizer,args,file_path=file_path,block_size=args.block_size)
...
def main():
    ...
    parser.add_argument("--vocab_path", type=str, default="", help="Path to the out cls vocab")
